"""Rescore all listings using CPK-based pricing as primary source."""
...
async def rescore_all_with_cpk():
    """Rescore all listings using CPK pricing."""
    async with AsyncSessionLocal() as db:
        ...
        SELECT
            ...
            lo.bid_count, lo.source, lo.observed_at, gs.cpk
        FROM gem_radar_listing_observations lo
        JOIN gem_radar_scored_listings gs ON lo.listing_id = gs.listing_id
        ...
        for i, row in enumerate(listings, 1):
            try:
                ...
                # Score with CPK enabled (pipeline will look up CPK internally)
                ...
                db_row = GemRadarScoredListing(
                    ...
                    search_run_id="rescore-cpk-final",
                    source=row[11],
                    url=listing.url,
                ...
                if i % 50 == 0:
                    ...
                    print(f"[{i:5d}/{total}] Priced: {priced_count:5d} ({pct}%) | CPK-priced: {cpk_priced:5d} | GEM: {gem:4d} | SUPER_GEM: {super_gem:4d} | Rate: {rate:.1f}/s")
        ...
        print(f"  Priced via CPK: {cpk_priced}")
        print(f"  GEM: {gem}")
