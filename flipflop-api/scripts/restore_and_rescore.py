"""Restore baseline scores and integrate CPK pricing."""
...
async def score_and_integrate_cpk():
    """Score all observations and integrate CPK pricing."""
    async with AsyncSessionLocal() as db:
        # Get all unscored observations
        query = """
        SELECT
        ...
        if not listings:
            print("All listings already scored.")
            return

        total = len(listings)
        print(f"Scoring {total} unscored observations...")
        print()
        ...
        for i, row in enumerate(listings, 1):
            try:
                ...
                db_row = GemRadarScoredListing(
                    ...
                    search_run_id="restore-baseline",
                    source=row[11],
                    url=listing.url,
                ...
                if i % 100 == 0:
                    ...
                    print(f"[{i:5d}/{total}] Priced: {priced_count:5d} ({pct}%) | GEM: {gem:4d} | SUPER_GEM: {super_gem:4d} | Rate: {rate:.1f}/s")

            except Exception as exc:
                log.error("score_failed", listing_id=row[0], error=str(exc))
                continue
