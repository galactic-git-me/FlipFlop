...
def test_priority_restricts_suppliers_stock_capacity_and_delivery():
    ...
    assert eligible_offers(offers, FulfilmentMode.PRIORITY, ConditionPolicy.NEW_ONLY, False) == []
    result = eligible_offers(offers, FulfilmentMode.PRIORITY, ConditionPolicy.NEW_ONLY, False, priority_capacity=True)
    assert [offer.supplier for offer in result] == ["Amazon"]
    standard = eligible_offers(offers, FulfilmentMode.STANDARD, ConditionPolicy.NEW_ONLY, False)
...
def test_true_cost_and_sold_market_gate_ignore_active_asking_price():
    ...
    result = evaluate_price(costs, market, Condition.NEW, D("100"), D("0.15"))
...
def test_market_gate_suppresses_uneconomic_build():
    ...
    result = evaluate_price(costs, market, Condition.NEW, D("100"), D("0.20"))
