...
async def run_delivery_followup_job() -> dict:
    ...
    async with AsyncSessionLocal() as db:
        ...
        for build in rows:
            ok = await send_delivery_followup_email(build.customer_email, build.buyer_name or "there", build.id, review_url=f"https://theflipflop.shop/reviews?build={build.id}")
            if ok:
