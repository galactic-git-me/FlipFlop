...
function MarketPrice({ variant: v }: { variant: Variant }) {
  const median = v.market_median_price;
  return <div className="min-w-0" title="Market range from the matched CPK comparable listings">
    ...
    <p className="text-[10px] text-slate-500">Market median{v.market_lower_price != null && v.market_upper_price != null ? ` · £${v.market_lower_price.toFixed(0)}–£${v.market_upper_price.toFixed(0)}` : ""}</p>
  </div>;
}
