...
export default function CuratedBuildsPage() {
  ...
  return <main className="min-h-screen bg-[#080d14] px-5 py-7 text-slate-100 md:px-8">
    <div className="mx-auto max-w-[1600px] space-y-6">
      ...
      {tab === "playbook" && <PlaybookGrid builds={draftBuilds} segments={segments} selectedId={selectedId} onSelect={setSelectedId} />}
...
function PlaybookGrid({ builds, segments, selectedId, onSelect }: { builds: DraftBuild[]; segments: Segment[]; selectedId: number | null; onSelect: (id: number) => void }) {
    ...
    {builds.length === 0 ? <Empty title="No playbook builds loaded" text="The draft playbook could not be loaded." /> : <div className="overflow-x-auto rounded-xl border border-white/10">
      <div className="min-w-[1100px]"><div className="grid grid-cols-[160px_repeat(3,minmax(0,1fr))] bg-[#14202d] text-sm font-semibold text-slate-300"><div className="px-4 py-3">Customer type</div>{tiers.map(tier => <div key={tier} className="px-4 py-3">{tier}</div>)}</div>
        {types.map(type => <div key={type} className="grid grid-cols-[160px_repeat(3,minmax(0,1fr))] border-t border-white/10"><h3 className="px-4 py-4 text-sm font-semibold text-cyan-200">{type}</h3>{tiers.map(tier => {
          ...
          const assigned = segment ? Object.values(segment.component_details).filter(Boolean).length : 0;
          return <button key={tier} type="button" onClick={() => segment && onSelect(segment.id)} disabled={!segment} className={`border-l border-white/10 px-4 py-4 text-left transition hover:bg-white/[.04] disabled:cursor-default ${selectedId === segment?.id ? "bg-cyan-400/[.08] ring-1 ring-inset ring-cyan-400/40" : ""}`}>
            ...
            <dl className="mt-3 space-y-1">{highlights.map(({ category, slot, label }) => { const title = segment?.component_details?.[slot]?.title ?? build.core_components.find(item => item.category === category)?.sku_name; return title ? <div key={category} className="grid grid-cols-[54px_1fr] gap-2 text-xs"><dt className="text-slate-500">{label}</dt><dd className={`line-clamp-1 ${segment?.component_details?.[slot] ? "text-emerald-200" : "text-slate-200"}`} title={title}>{title}</dd></div> : null; })}</dl>
            <p className="mt-3 text-[11px] text-slate-500">{assigned > 0 ? `${assigned} curated catalogue parts assigned` : "Draft specification · catalogue parts pending"}</p>
          </button>;
        })}</div>)}</div></div>}
  </section>;
}
