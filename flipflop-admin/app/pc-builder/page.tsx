...
type BestsellerMatch = { category: string; rank: number; asin: string; cpk: string; title: string; url: string | null; image_url: string | null; price: number | null; marketplace_listing_id: string; marketplace_source: string; marketplace_title: string; marketplace_url: string | null; marketplace_price: number; marketplace_condition: string | null; marketplace_seen_at: string; review_status: "pending" | "approved" | "rejected" };
...
type Segment = { id: number; customer_type: string; budget_level: string; budget_min?: number; budget_max?: number; components: Record<string, number>; bestseller_components: Record<string, { category: string; cpk: string }>; component_details: Record<string, Component | null>; component_cost: number; selling_price?: number; proposed_selling_price?: number; availability_status: string; is_live: boolean; regeneration_status: string };
...
export default function CuratedBuildsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (bestsellerResponse.ok) setBestsellerMatches((await bestsellerResponse.json()).items ?? []);
  ...
  return <main className="min-h-screen bg-[#080d14] px-5 py-7 text-slate-100 md:px-8">
    <div className="mx-auto max-w-[1600px] space-y-6">
      ...
      {tab === "segments" && <section className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Customer types and budget levels</h2><p className="text-sm text-slate-400">Create one playbook build for each audience and budget combination.</p></div><button onClick={() => { const name = window.prompt("Customer type (e.g. Gaming)"); if (!name?.trim()) return; const budget = window.prompt("Budget level (e.g. Entry, Mid-range, Premium)"); if (budget?.trim()) void saveSegment({ customer_type: name.trim(), budget_level: budget.trim(), components: {} }); }} className="inline-flex items-center gap-2 rounded-lg bg-cyan-500 px-3 py-2 text-sm font-semibold text-slate-950"><Plus className="h-4 w-4"/>Add segment</button></div>
        {segments.length === 0 ? <Empty title="No customer segments yet" text="Add your first customer type and budget level to start the playbook."/> : <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{segments.map(s => { const draft = draftBuilds.find(b => b.customer_type === s.customer_type && b.budget_tier === s.budget_level); return <button key={s.id} onClick={() => { setSelectedId(s.id); setTab("playbook"); }} className="rounded-xl border border-white/10 bg-[#101924] p-4 text-left hover:border-cyan-400/40"><div className="flex justify-between gap-3"><div><p className="font-semibold">{s.customer_type}</p><p className="mt-1 text-sm text-slate-400">{s.budget_level}{draft ? ` · ${draft.name}` : ""}</p></div><Badge tone={s.is_live ? "green" : "amber"}>{s.is_live ? "Live" : "Draft"}</Badge></div><p className="mt-4 text-xs text-slate-500">{draft ? `Indicative £${draft.indicative_sell_gbp.toLocaleString()} · ` : ""}{Object.keys(s.bestseller_components ?? {}).length} reviewed bestseller products linked</p></button>; })}</div>}</section>}
