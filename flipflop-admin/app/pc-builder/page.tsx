...
type BestsellerMatch = { category: string; rank: number; asin: string; cpk: string; title: string; url: string | null; image_url: string | null; price: number | null; marketplace_listing_id: string; marketplace_source: string; marketplace_title: string; marketplace_url: string | null; marketplace_image_url: string | null; marketplace_image_is_reference: boolean; marketplace_is_self_capture: boolean; marketplace_price: number | null; marketplace_condition: string | null; marketplace_seen_at: string; review_status: "pending" | "approved" | "rejected" };
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-2 backdrop-blur-sm sm:p-5" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div role="dialog" aria-modal="true" aria-labelledby="match-review-title" className="flex h-[min(96vh,1100px)] w-full max-w-[1800px] flex-col overflow-hidden rounded-2xl border border-white/15 bg-[#0b1119] shadow-2xl">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-px overflow-y-auto bg-white/10 md:grid-cols-2 md:overflow-hidden">
        ...
        <MatchImagePanel key={`market-${match.marketplace_listing_id}`} label={match.marketplace_image_is_reference ? `${match.marketplace_source} · product reference photo` : match.marketplace_is_self_capture ? "Amazon bestseller capture · ASIN self-match" : `${match.marketplace_source} listing`} title={match.marketplace_title} price={match.marketplace_price} imageUrl={match.marketplace_image_url} url={match.marketplace_url} detail={match.marketplace_condition ?? "Condition unknown"} />
      </div>
...
function PlaybookGrid({ builds, segments, matches, selectedId, onSelect }: { builds: DraftBuild[]; segments: Segment[]; matches: BestsellerMatch[]; selectedId: number | null; onSelect: (id: number) => void }) {
  const types = [...new Set(builds.map(build => build.customer_type))];
    ...
    {builds.length === 0 ? <Empty title="No playbook builds loaded" text="The draft playbook could not be loaded." /> : <div className="overflow-x-auto rounded-xl border border-white/10">
      <div className="min-w-[1100px]"><div className="grid grid-cols-[160px_repeat(3,minmax(0,1fr))] bg-[#14202d] text-sm font-semibold text-slate-300"><div className="px-4 py-3">Customer type</div>{tiers.map(tier => <div key={tier} className="px-4 py-3">{tier}</div>)}</div>
        {types.map(type => <div key={type} className="grid grid-cols-[160px_repeat(3,minmax(0,1fr))] border-t border-white/10"><h3 className="px-4 py-4 text-sm font-semibold text-cyan-200">{type}</h3>{tiers.map(tier => {
          const build = builds.find(item => item.customer_type === type && item.budget_tier === tier);
          const segment = segments.find(item => item.customer_type === type && item.budget_level === tier);
          if (!build) return <div key={tier} className="px-4 py-4 text-sm text-slate-500">No build</div>;
          const assigned = Object.keys(segment?.bestseller_components ?? {}).length;
          return <button key={tier} type="button" onClick={() => segment && onSelect(segment.id)} disabled={!segment} className={`border-l border-white/10 px-4 py-4 text-left transition hover:bg-white/[.04] disabled:cursor-default ${selectedId === segment?.id ? "bg-cyan-400/[.08] ring-1 ring-inset ring-cyan-400/40" : ""}`}>
            <div className="flex items-start justify-between gap-2"><span className="font-semibold text-white">{build.name}</span><span className="whitespace-nowrap text-sm font-semibold text-cyan-200">£{build.indicative_sell_gbp.toLocaleString()}</span></div>
            <p className="mt-1 line-clamp-2 text-xs text-slate-400">{build.note}</p>
            <dl className="mt-3 space-y-1">{highlights.map(({ category, slot, label }) => { const ref = segment?.bestseller_components?.[slot]; const match = ref && matches.find(item => item.category === ref.category && item.cpk === ref.cpk); const title = match?.title ?? build.core_components.find(item => item.category === category)?.sku_name; return title ? <div key={category} className="grid grid-cols-[54px_1fr] gap-2 text-xs"><dt className="text-slate-500">{label}</dt><dd className={`line-clamp-1 ${match ? "text-emerald-200" : "text-slate-200"}`} title={title}>{title}</dd></div> : null; })}</dl>
            <p className="mt-3 text-[11px] text-slate-500">{assigned > 0 ? `${assigned}/8 Amazon bestseller components assigned` : "Draft specification · bestseller choices pending"}</p>
          </button>;
        })}</div>)}</div></div>}
  </section>;
}
