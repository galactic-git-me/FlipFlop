...
import { AlertTriangle, Check, ExternalLink, Loader2, RefreshCw, ShieldCheck, TrendingUp } from "lucide-react";
  ...
  return <div className="space-y-5">
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {[
        ...
        ["Parts market value", data.component_resale_total, "Current component values; no depreciation"],
        [hasMarketEvidence ? "Recommended list price" : "Provisional list price", r.recommended_price, hasMarketEvidence ? `${r.confidence} confidence` : "Cost and margin fallback; no market comps"],
        ["Protected offer floor", r.floor_price, `After ${r.fee_rate_pct}% configured fees`],
      ].map(([label, value, detail]) => <button type="button" onClick={() => setDetailModal(label === "Actual build cost" ? "cost" : label === "Parts market value" ? "resale" : label === "Recommended list price" || label === "Provisional list price" ? "market" : null)} key={String(label)} className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-4 text-left transition-colors hover:border-cyan-400/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">
    ...
    <section className="rounded-xl border border-cyan-400/15 bg-cyan-400/[0.035] p-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="flex items-center gap-2"><TrendingUp className="h-4 w-4 text-cyan-400" /><h2 className="font-semibold">{hasMarketEvidence ? "Market recommendation" : "Provisional pricing"}</h2>{hasMarketEvidence && <Confidence value={r.confidence} />}</div>
          {hasMarketEvidence ? <button type="button" onClick={() => setDetailModal("market")} className="mt-2 cursor-pointer text-left text-sm text-slate-300 hover:text-cyan-300">Range {formatCurrency(r.market_low)}–{formatCurrency(r.market_high)} · midpoint {formatCurrency(r.market_mid)} · view evidence</button> : <p className="mt-2 text-sm font-semibold text-amber-300">No sourced market range is available.</p>}
          <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">{hasMarketEvidence ? r.rationale : "The displayed provisional price is derived from component values and the fee-aware margin floor, not comparable PC sales"}. The floor protects a 10% margin after configured marketplace fees.</p>
        </div>
